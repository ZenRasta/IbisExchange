...
function TabBar() {
  ...
  // Hide tabs on trade detail page and KYC page
  const hiddenPaths = ['/trade/', '/kyc'];
...
function BackButtonHandler() {
  ...
  useEffect(() => {
    const isRootPage = ['/', '/my-trades', '/create', '/profile'].includes(location.pathname);
...
function Header() {
  return (
    <div className="fixed top-0 left-0 right-0 bg-tg-bg z-50 border-b border-tg-secondary-bg">
      <div className="flex items-center justify-between px-4 h-12">
        <span className="text-tg-text font-semibold text-lg">Ibis Exchange</span>
        <TonConnectButton />
      </div>
    </div>
  );
