...
export default function OrderBook() {
  ...
  const fetchOrders = useCallback(async (isRefresh = false) => {
    ...
    try {
      const params = new URLSearchParams({ type: tab, sort, currency });
  ...
  }, [tab, sort, filterMethod, currency]);

  useEffect(() => {
    fetchOrders();
  }, [fetchOrders]);

  // Reset filter when currency changes since payment methods differ
  useEffect(() => {
  ...
  return (
    <div className="px-4 py-4 animate-fade-in">
      {/* Currency Tabs */}
      <div className="flex overflow-x-auto no-scrollbar gap-1.5 mb-4 -mx-1 px-1">
      ...
      <div className="flex bg-tg-section-bg rounded-xl p-1 mb-4">
        <button
          onClick={() => { setViewMode('list'); impact('light'); }}
          className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${
            viewMode === 'list' ? 'bg-tg-button text-tg-button-text' : 'text-tg-hint'
          }`}
        >
          List View
        </button>
        <button
          onClick={() => { setViewMode('depth'); impact('light'); }}
          className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${
            viewMode === 'depth' ? 'bg-tg-button text-tg-button-text' : 'text-tg-hint'
          }`}
        >
          Depth Chart
        </button>
      ...
      {viewMode === 'depth' ? (
        <div className="bg-tg-section-bg rounded-2xl p-4 mb-4">
          {loading ? (
            ...
            <DepthChart orders={orders} currency={currency} />
          )}
        </div>
      ) : (
        <>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-tg-hint text-sm mt-1">
                Be the first to create a {tab.toLowerCase()} order for {currency}
              </p>
              <button
                onClick={() => navigate(`/create?type=${tab}&currency=${currency}`)}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-tg-accent text-sm font-semibold">{order.pricePerUsdt} {currencySymbol}</p>
                      </div>
                    </div>

                    <div className="flex items-center justify-between">
                      <p className="text-tg-hint text-xs">
                        Total: <span className="text-tg-text">{(order.remainingAmount * order.pricePerUsdt).toLocaleString()} {currencySymbol}</span>
                      </p>
